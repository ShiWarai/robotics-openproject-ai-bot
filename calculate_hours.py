...
from create_task import CustomCalendar
...
def get_time_entries(user_id: str, start_date: str, end_date: Optional[str] = None) -> Optional[List[Dict]]:
    ...
    def get_time_entries(user_id: str, start_date: str, end_date: Optional[str] = None) -> Optional[List[Dict]]:
        """Получает записи о времени для пользователя с указанной даты."""
        url = f"{OP_API_URL}/time_entries"
        headers = {"Content-Type": "application/json"}

        # Преобразуем даты в формат ISO 8601
        start_date_iso = f"{start_date}T00:00:00Z"
        end_date_iso = f"{end_date}T23:59:59Z" if end_date else None

        filters = [
            {"user": {"operator": "=", "values": [user_id]}},
            {"spentOn": {"operator": ">=", "values": [start_date_iso]}}
        ]
        if end_date_iso:
            filters.append({"spentOn": {"operator": "<=", "values": [end_date_iso]}})

        logger.info(f"Формируем фильтр: {filters}")
        params = {
            "filters": json.dumps(filters)
        }
        logger.info(f"Отправляем запрос с параметрами: {params}")
